...
- **Phase 8**: Integrated Frontend with Backend V1.6.2 for improved Obligation Payments UX. Implemented realtime FX previews powered entirely by the backend preview endpoint. Replaced the confusing FIFO/"Pagar obligación" button with a unified "Pagar periodo" flow. Added support for displaying and paying the `remaining_amount` exactly as provided by the backend. Mapped specific payment errors (overpayment, unsupported fx, insufficient funds) to clear UX messages without introducing local financial math.
